...
const float LOW_MIN = 0.10;
const float LOW_MAX = 0.30;
const float HIGH_MIN = 0.70;
const float HIGH_MAX = 0.90;
...
bool processSensor(int analogPin, int ledPin, const char* label) {
  ...
  if (ratio >= LOW_MIN && ratio <= LOW_MAX) {
    Serial.print("LOW)");
    digitalWrite(ledPin, LOW);
    return false;
  } else if (ratio >= HIGH_MIN && ratio <= HIGH_MAX) {
    Serial.print("HIGH)");
